...
async def login_user(
    realm: str, user: UserLogin, response: Response, provider: Depends[UserCredentialsProvider]
) -> TokenPair:
...
async def introspect_token(
    realm: str, token: TokenIntrospect, request: Request, service: Depends[UserTokenService]
) -> UserClaims:
    session_id = request.cookies.get("session_id")
    return await service.introspect(token.token, realm=realm, session_id=UUID(session_id))
...
async def switch_realm(
    realm: str, user: UserRealmSwitch, request: Request, service: Depends[UserTokenService]
) -> TokenPair:
    session_id = request.cookies.get("session_id")
    if session_id is None:
        raise HTTPException(
